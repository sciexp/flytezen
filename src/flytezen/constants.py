...
if repo_root:
    REMOTE_CLUSTER_CONFIG_FILE_PATH = os.path.join(repo_root, ".flyte", "config.yaml")
    LOCAL_CLUSTER_CONFIG_FILE_PATH = os.path.join(repo_root, ".flyte", "config-local.yaml")
    ...
    logger.debug(f"Remote cluster config file path: {REMOTE_CLUSTER_CONFIG_FILE_PATH}")
    logger.debug(f"Local cluster config file path: {LOCAL_CLUSTER_CONFIG_FILE_PATH}")
else:
    git_repo_not_found = "Not inside a Git repository or Git is not installed."
    raise OSError(git_repo_not_found)
